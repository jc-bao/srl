...
def compute_franka_reward(
  ...
  rewards = torch.where(
    cabinet_dof_pos[:, 3] > 0.39, rewards +
    (2.0 * around_handle_reward), rewards
  )
...
if __name__ == '__main__':
  ...
  '''
  import yaml
  with open('/home/reed/rl/ElegantRL/elegantrl/envs/isaac_configs/FrankaCabinet.yaml') as config_file:
    task_config = yaml.load(config_file, Loader=yaml.SafeLoader)
  print(task_config)
  env = FrankaCabinet(cfg=task_config, sim_device='cuda:0',
