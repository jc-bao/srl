...
class FrankaCube(gym.Env):
  ...
  def _create_envs(self):
    ...
    self.franka_default_dof_pos = to_torch(
      [-0.4050, 0.2139, -0.2834, -2.3273,  0.1036,
        2.5304,  1.5893,  0.0200, 0.0200],
      device=self.device,)
  ...
  def reset(self):
    for _ in range(10):
      self.reset_buf[:] = True
      act = torch.zeros((self.cfg.num_envs, 4),
                        device=self.device, dtype=torch.float)
      obs, _, _, _ = self.step(act, use_init_pos=True)
  ...
  def pre_physics_step(self, actions, use_init_pos=False):
    ...
    if done_env_num > 0:
      ...
      if random_idx.any():
        self.block_states[random_idx, :, :3] = self.torch_block_space.sample(
          (random_idx.sum(),))
      if inhand_idx.any():
        self.block_states[inhand_idx, torch.randint(
          self.cfg.num_goals, (1,), device=self.device)[0], :3] = self.grip_pos[inhand_idx]
    ...
    self.franka_dof_targets[reset_idx & use_init_pos,
                            :self.num_franka_dofs] = self.franka_default_dof_pos
    # Deploy actions
...
if __name__ == '__main__':
  ...
  '''
	run random policy
	'''
  env = gym.make('PandaPNP-v0', num_envs=4, num_cameras=0, headless=False)
  obs = env.reset()
  start = time.time()
  while True:
